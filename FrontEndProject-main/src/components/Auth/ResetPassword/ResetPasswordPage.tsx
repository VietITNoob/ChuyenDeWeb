          ...
          <>
            <div className="opacity-0 translate-y-5 animate-[fadeInUp_0.8s_cubic-bezier(0.25,0.8,0.25,1)_0.1s_forwards]">
              ...
              <h2 className="text-[21px] font-normal text-apple-gray mb-[50px]">
                Tạo mật khẩu mới cho tài khoản <br/><b className="text-apple-dark">{userEmail}</b>
              </h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="opacity-0 translate-y-5 animate-[fadeInUp_0.8s_cubic-bezier(0.25,0.8,0.25,1)_0.2s_forwards]">
              <div className="max-w-[440px] mx-auto mb-10 relative text-left">
                ...
                <div className={`overflow-hidden transition-all duration-500 origin-top ${password.length > 0 ? 'max-h-[100px] opacity-100 translate-y-0 mt-4' : 'max-h-0 opacity-0 -translate-y-2.5'}`}>
                    ...
                    <button 
                        type="submit" 
                        className={`w-10 h-10 rounded-full border-none flex items-center justify-center cursor-pointer mr-3.5 transition-all duration-400 overflow-hidden ${isMatch && isValidLength ? 'bg-apple-blue text-white hover:bg-[#0077ed] hover:scale-110 hover:shadow-[0_4px_12px_rgba(0,113,227,0.3)]' : 'bg-[#e5e5e5] text-[#888]'}`}
                        disabled={!isMatch || !isValidLength || loading}
                    >
                        {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <ArrowRight size={20} className={`transition-transform duration-300 ${isMatch && isValidLength ? 'group-hover:translate-x-[3px]' : ''}`} />}
                    </button>
                    </div>
                </div>
              </div>
              ...
              {error && <p className="text-[#ff3b30] text-center mt-2.5">{error}</p>}
            </form>
          </>
        ) : (
