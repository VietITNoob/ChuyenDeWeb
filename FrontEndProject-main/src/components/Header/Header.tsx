...
const Header = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-[9999] h-[52px] font-sans transition-all duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${
      ...
      >
        <div className="max-w-[1024px] mx-auto h-full px-[22px] flex justify-between items-center relative z-20 transition-opacity duration-300">
          ...
          <ul className="hidden md:flex gap-1 list-none m-0 p-0 h-full items-center">
            {HEADER_MENU.map((item) => (
              ...
              >
                <Link 
                  to={item.path || '#'} 
                  className={`text-[12px] font-normal no-underline px-3 py-2 rounded-lg transition-all duration-300 ${
                    hoveredNavItem === item.id ? 'opacity-100 bg-black/5 text-black' : 'opacity-80 text-apple-dark group-hover:opacity-100 group-hover:bg-black/5 group-hover:text-black'
                  }`}
                >
                  {item.label}
                </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {/* Cart Popover */}
              <div className={`absolute top-[calc(100%+15px)] -right-5 w-[320px] bg-white rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] border border-black/10 p-4 z-[1500] transition-all duration-300 text-apple-dark ${
                showCartPopover ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2.5'
              }`}>
                {lastAddedItem && (
                  <>
            ...
            <div className="relative">
              ...
              {/* Wishlist Popover */}
              <div className={`absolute top-[calc(100%+15px)] -right-5 w-[320px] bg-white rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] border border-black/10 p-4 z-[1500] transition-all duration-300 text-apple-dark ${
                showWishlistPopover ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2.5'
              }`}>
                {lastWishlistItem && (
                  <>
            ...
            >
              {isAuthenticated && user ? (
                <div className="flex items-center gap-2 cursor-pointer text-apple-dark text-[14px] font-medium py-1 group">
                   <span className="hidden md:inline">Hi, {user.lastName}</span>
                   <UserCircle size={24} className="text-[#555] group-hover:text-apple-blue transition-colors duration-300" />
                   
                   <div className={`absolute top-full -right-2.5 w-[240px] bg-white rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.15)] py-2.5 border border-black/5 z-[1000] transition-all duration-200 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${
                     showUserMenu ? 'opacity-100 visible translate-y-[15px]' : 'opacity-0 invisible translate-y-2.5'
                   }`}>
                      ...
                      <div className="px-5 py-2.5 border-b border-[#f0f0f0] mb-1">
                        <p className="font-semibold text-apple-dark m-0 text-[15px]">{user.firstName} {user.lastName}</p>
        ...
        {/* 5. SEARCH DROPDOWN */}
        <div className={`absolute top-[52px] left-0 right-0 bg-white/98 backdrop-blur-[20px] border-b-4 border-black/10 shadow-[0_20px_40px_rgba(0,0,0,0.05)] rounded-lg overflow-hidden z-10 flex justify-center items-start transition-all duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${
          isSearchOpen ? 'visible h-[340px] opacity-100 delay-0' : 'invisible h-0 opacity-0 delay-500'
        }`}>
          <div className={`w-full max-w-[680px] py-5 flex flex-col gap-5 transition-all duration-400 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${
            isSearchOpen ? 'opacity-100 translate-y-0 delay-100' : 'opacity-0 -translate-y-2.5'
          }`}>
      ...
      {/* BACKDROP */}
      <div 
        className={`fixed top-[52px] left-0 right-0 bottom-0 bg-black/40 backdrop-blur-[4px] z-[9998] transition-opacity duration-300 ${
